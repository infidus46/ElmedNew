...
namespace Yamed.Hospital
{
    ...
    //    public object Convert(object[] values, Type targetType, object parameter, System.Globalization.CultureInfo culture)
    //    {
    //        if ((bool) values[1] == false || values[0] == DependencyProperty.UnsetValue) return Brushes.Black;
    //        PolisInfo webPolicyStatus;
    //        using (WaitForm.AsyncWaitDialog.ShowWaitDialog("Он-лайн проверка на статус полиса"))
    //        {
    //            webPolicyStatus = null; //ImportExportData.GetWebPolicyStatus((string)values[0], 0);
    //        }

    //        if (webPolicyStatus != null)
    //        {
    //            if (webPolicyStatus.isValid)
    //            {
    ...
    //        //else if (values[0] != null && (values[0] as Nullable<DateTime>) > DateTime.Now) return new SolidColorBrush(Colors.Red);
    //        //else return new SolidColorBrush(Colors.Red);

    //        //if (values[0] == null && values[1] == null) return null;
    //        //if (values[1] != null) return new SolidColorBrush(Colors.Red);
    //        //if (values[0] != null && (values[0] as Nullable<DateTime>) < DateTime.Now) return new SolidColorBrush(Colors.Red);
    //        //else return null;
    //        //    else if (value[1] != null)
    //        //    {
    //        //        return ((DateTime)value[1]).Date < DateTime.Now.Date
